    ...
    for iter in range(max_iters):
        ...
        for tensors_during, tensors_discard, tensors_post in dataloader:
            if verbose is True:
                ...
                print(
                    f"tensors_during: shape = Tuple({tensors_during[0].shape}, Tuple({tensors_during[1][0].shape}, {tensors_during[1][1].shape}, {tensors_during[1][2].shape}), {tensors_during[2].shape}, {tensors_during[3].shape})"
                )

                # print the shape of each of the tensors in tensors_discard
                print(
                    f"tensors_discard: shape = Tuple({tensors_discard[0].shape}, Tuple({tensors_discard[1][0].shape}, {tensors_discard[1][1].shape}, {tensors_discard[1][2].shape}), {tensors_discard[2].shape}, {tensors_discard[3].shape})"
                )

                # print the shape of each of the tensors in tensors_post
                print(
                    f"tensors_post: shape = Tuple({tensors_post[0].shape}, Tuple({tensors_post[1][0].shape}, {tensors_post[1][1].shape}, {tensors_post[1][2].shape}), {tensors_post[2].shape}, {tensors_post[3].shape}"
                )
